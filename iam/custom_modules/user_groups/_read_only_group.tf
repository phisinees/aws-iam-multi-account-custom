...
resource "aws_iam_group_membership" "readonly_group" {
...
resource "aws_iam_group_policy_attachment" "readonly_group" {
  group      = aws_iam_group.readonly_group.name
  policy_arn = aws_iam_policy.readonly_group.arn
}

resource "aws_iam_policy" "readonly_group" {
  name   = "readonly_group_policy"
  policy = data.aws_iam_policy_document.readonly_group.json
}

data "aws_iam_policy_document" "readonly_group" {
  statement {
    effect  = "Allow"
